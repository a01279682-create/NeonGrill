...
// Place this file at: script.js
(() => {
  // Sample menu items (id, name, price (number), emoji, category, description)
  const menuItems = [
  ...
  const saveCart = () => {
    try { localStorage.setItem('neon_cart', JSON.stringify(cart)); } catch (e) { /* ignore */ }
  };
  const loadCart = () => {
    try {
  ...
  // Escape HTML helper to avoid injection in descriptions
  ...
  // Update quantity in menu card input (keeps UI synced after cart changes)
  ...
  // Wire up events (delegation where sensible)
  function bindEvents() {
    // Filter buttons
    ...
    // Keyboard support for filters (Enter/Space)
    ...
    // Menu grid interactions (increase/decrease/add via delegation)
    ...
    menuGrid.addEventListener('change', (ev) => {
      ...
      if (v === 0) {
        // do nothing until Add pressed — keep the input only
        input.value = 0;
      } else {
        // update the cart directly when the user types a positive number
        cart[id] = v;
        saveCart();
        renderCart();
      }
    ...
    confirmOrderBtn && confirmOrderBtn.addEventListener('click', () => {
      // simple confirmation: show modal and clear cart
      if (modal) {
      ...
      setTimeout(() => closeModalBtn && closeModalBtn.focus(), 120);
    });
  ...
  function openCart() {
    ...
    // move focus into cart for keyboard users
    const firstClose = cartEl.querySelector('.close');
  ...
  function init() {
    ...
    // If there is a skip link, ensure it jumps to the first main section
